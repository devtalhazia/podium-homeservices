...
/* ── SVG Quote Marks ──────────────────────────────────────── */
const QuoteSVG = () => (
  <svg role="presentation" aria-hidden="true" viewBox="0 0 26.042 22.546" overflow="visible" className="w-[26px] h-[23px] shrink-0">
    <path
...
const QuoteMedCard = ({ image, quote, author, company }: QuoteMedCardProps) => (
  ...
  >
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      <img src={`/images/${image}`} alt="" aria-hidden="true" decoding="async" style={{ display: 'block', width: '100%', height: '100%', objectFit: 'cover', borderRadius: 'inherit' }} />
    </div>
    <motion.div
      aria-hidden="true"
      variants={{ rest: { top: 0 } }}
      transition={{ duration: 0.45, ease: EASE }}
      style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 0, background: 'linear-gradient(0deg, rgba(0, 0, 0, 0.85) 0%, rgba(0, 0, 0, 0) 100%)', height: 504 }}
    />
    <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'row', flex: '1 0 0', alignItems: 'flex-start', justifyContent: 'flex-end', gap: 48 }}>
      <QuoteSVG />
      <div style={{ display: 'flex', flexDirection: 'column', flex: '1 0 0', gap: 24, alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '100%' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, width: '100%' }}>
            <p style={{ fontFamily: '"Graphik Regular", sans-serif', fontSize: 18, fontWeight: 400, lineHeight: '150%', color: 'rgb(255, 255, 255)', margin: 0 }}>{author}</p>
            <p style={{ fontFamily: '"Graphik Semibold", sans-serif', fontSize: 18, fontWeight: 600, lineHeight: '150%', color: 'rgb(255, 255, 255)', margin: 0 }}>{company}</p>
          </div>
        </div>
      </div>
    </div>
...
const GlassSmallCard = ({ background, icon, stat, statFontSize, label, company }: GlassSmallCardProps) => (
  ...
  >
    ...
    <motion.div
      ...
      style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 0, transform: 'rotate(-180deg)', background: 'linear-gradient(0deg, rgb(0,0,0) 0%, rgba(0,0,0,0.1) 100%)' }}
    />
...
/* ── Card Type B: GlassColumn ────────────────────────────── */
interface GlassColumnProps { top: GlassSmallCardProps; bottom: GlassSmallCardProps }
...
/* ── Card Type C: Single Tall Glass Stat ─────────────────── */
interface StatCardProps { background: string; icon: string; stat: string; statFontSize: number; detail: string }
const StatCard = ({ background, icon, stat, statFontSize, detail }: StatCardProps) => (
  <motion.div
    whileHover={{ scale: 1.02 }}
    transition={{ duration: 0.3, ease: EASE }}
    style={{
      position: 'relative', width: 320, height: 504, borderRadius: 24, overflow: 'hidden', cursor: 'pointer',
      background: 'linear-gradient(180deg, rgba(61, 57, 53, 0.06) 0%, rgba(126, 120, 104, 0.06) 100%)',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 30, padding: 40, flexShrink: 0,
    }}
  >
    <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
      <img src={`/images/${background}`} alt="" aria-hidden="true" decoding="async" style={{ display: 'block', width: '100%', height: '100%', objectFit: 'cover' }} />
    </div>
    <div aria-hidden="true" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: -400, zIndex: 0, background: 'linear-gradient(0deg, rgb(0,0,0) 0%, rgba(0,0,0,0.1) 100%)', opacity: 0.25, transform: 'rotate(-180deg)' }} />
...
/* ── Card Type D: CustomerStoryCard ──────────────────────── */
interface CustomerStoryCardProps { image1: string; image2: string }
...
const CAROUSEL_ITEMS = [
  <QuoteMedCard key="q1" image="JXAwZLcvNuN9FwACgddrp1qCU.png" quote="When someone's AC is out in Louisiana, they want service NOW. If we respond first, we win the business." author="Carl Kelly, COO" company="Trouth AC" />,
  <GlassColumn key="gc1"
...
function InfiniteMarquee() {
  ...
  return (
    <div
      className="order-[1] w-full overflow-hidden relative cursor-grab"
      onMouseEnter={() => { isPaused.current = true }}
      onMouseLeave={() => { isPaused.current = false }}
    >
      <motion.div style={{ x, display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: 10, width: 'max-content', paddingLeft: 10 }}>
        {CAROUSEL_ITEMS}
...
const cardTitleStyle = {
  ...
  fontSize: 20, fontWeight: 600, lineHeight: '150%', letterSpacing: 0, textAlign: 'center' as const,
  color: 'rgb(247, 244, 237)', margin: 0, whiteSpace: 'pre-wrap' as const, wordBreak: 'break-word' as const, width: '100%',
}
const cardBodyStyle = {
  fontFamily: '"Graphik Regular", "Graphik Regular Placeholder", sans-serif',
  fontSize: 15, fontWeight: 400, lineHeight: '150%', letterSpacing: 0, textAlign: 'center' as const,
  color: 'rgb(170, 170, 173)', margin: 0, whiteSpace: 'pre-wrap' as const, wordBreak: 'break-word' as const, width: '100%',
}
...
export default function TestimonialsSection() {
  return (
    ...
    >
      ...
      {/* Feature cards row */}
      <div className="order-[3] flex flex-col tablet:flex-wrap tablet:flex-row items-center justify-start tablet:justify-between w-full max-w-[1600px] gap-[8px] tablet:gap-0 px-[24px] tablet:px-[48px] desktop:px-[80px] box-border">
        {/* Card 1 */}
        <motion.div
          className="rounded-[32px] w-full tablet:flex-[1_0_0] tablet:min-w-[204px] tablet:max-w-[347px] desktop:min-w-[340px] desktop:max-w-[460px] overflow-hidden"
          initial={{ opacity: 0, y: -20 }}
          whileInView={{ opacity: 1, y: 0 }}
        ...
        {/* Divider 1 */}
        <div className="shrink-0 w-full h-[2px] tablet:w-[3px] tablet:h-[208px] desktop:h-[250px]">
          <svg className="hidden tablet:block w-full h-full" viewBox="0 0 2 321" aria-hidden="true">
        ...
        >
          <div className="flex flex-col items-center justify-center gap-[16px] w-full p-[44px_28px] desktop:p-[48px_32px] box-border">
            <h6 style={cardTitleStyle}>Dedicated customer support</h6>
            <p style={cardBodyStyle}>Whatever your business needs, our expert support team is here to help you so you can focus on what you do best.</p>
          </div>
        </motion.div>
      ...
      {/* CTA row */}
      <motion.div
        className="order-[2] tablet:order-[3] flex flex-row items-center justify-center gap-[48px] w-full max-w-[1440px] px-[24px] tablet:px-[48px] desktop:px-[80px] box-border"
      ...
      >
        <a
          ...
          className="inline-flex flex-row items-center justify-center gap-[10px] px-[32px] py-[16px] bg-crimson rounded-[50px] no-underline cursor-pointer whitespace-nowrap"
        >
          <span className="font-graphik-semi text-[18px] font-semibold leading-[18px] text-cream text-center">
            Grow with us
          </span>
        </a>
      </motion.div>
